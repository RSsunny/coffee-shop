    ...
    return (
        <div className="text-black grid grid-cols-3 md:grid-cols-4 gap-5 md:gap-10 items-center justify-between bg-[#F5F4F1] p-3 md:p-8">
            ...
            <div className="text-white  flex flex-col items-end">
                <Link to={`/coffees/${_id}`}><AiOutlineEye className="bg-[#D2B48C] text-lg w-10 h-8 p-1 my-1 rounded-md cursor-pointer"></AiOutlineEye></Link>
                <Link to={`/update/${_id}`}><BsPencil className="bg-black text-lg w-10 h-8 p-2 my-1 rounded-md cursor-pointer"></BsPencil></Link>
                <AiOutlineDelete onClick={()=>handleDelet(_id)} className="bg-red-500 text-lg w-10 h-8 p-1 my-1 rounded-md cursor-pointer"></AiOutlineDelete>
