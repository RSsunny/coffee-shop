...
const UserList = ({ user, totaluser, setTotaluser, count }) => {
  ...
  const handledelete = (_id) => {
    fetch(
      `https://coffee-shop-server-2-q6b79vhaz-rabius-sunnys-projects.vercel.app/users/${_id}`,
      {
        method: "DELETE",
      }
    )
  ...
  return (
    <tr>
      ...
      <td className="text-xs md:text-base  border">{email}</td>
      ...
      <th className="hidden md:block">
        <div className="flex items-center justify-center gap-5 text-xl">
          ...
          <Link to={`/updateUsers/${_id}`}>
            <BsPencil className="hover:bg-[#015309] hover:text-white w-8 h-8 p-1 rounded-full cursor-pointer"></BsPencil>
          </Link>
          <RiDeleteBinLine
